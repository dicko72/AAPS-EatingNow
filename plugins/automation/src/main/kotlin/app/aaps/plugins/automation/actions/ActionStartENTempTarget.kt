...
class ActionStartENTempTarget(injector: HasAndroidInjector) : Action(injector) {
    ...
    override fun friendlyName(): Int = R.string.startentemptarget
    override fun shortDescription(): String = rh.gs(R.string.startentemptarget) + ": " + tt().friendlyDescription(value.units, rh, profileUtil)
    ...
    override fun doAction(callback: Callback) {
        disposable += repository.runTransactionForResult(InsertAndCancelCurrentTemporaryTargetTransaction(tt()))
            .subscribe({ result ->
                           result.inserted.forEach { aapsLogger.debug(LTag.DATABASE, "Inserted EN temp target $it") }
                           result.updated.forEach { aapsLogger.debug(LTag.DATABASE, "Updated EN temp target $it") }
                           uel.log(
                               UserEntry.Action.TT, Sources.Automation, title,
                               ValueWithUnit.TherapyEventTTReason(TemporaryTarget.Reason.EATING_NOW),
    ...
    private fun tt() = TemporaryTarget(
        ...
        reason = TemporaryTarget.Reason.EATING_NOW,
