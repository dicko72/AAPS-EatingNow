...
//    }
    ...
    ENWBolusIOBMax = (ENWindowOK && ENWindowRunTime <= ENWindowDuration ? ENWBolusIOBMax : 0); // reset to 0 if not within ENW
    ...
    if (ENactive || TIRB_sum > 1 || !ENtimeOK && eventualBG > target_bg && !HighTempTargetSet) {
        ...
        if (sens_predType.startsWith("UAM+")) {
            ...
            if (sens_predType == "UAM++") {
                ...
                minGuardBG = Math.max(minGuardBG,threshold);
                minBG = Math.max(minPredBG,minGuardBG); // go with the largest value for UAM+
                eventualBG = (ENWBolusIOBMax > 0 && meal_data.ENWBolusIOB < ENWBolusIOBMax ? Math.max(eventualBG,bg,bg+delta*3) : eventualBG); // if ENW IOB is below the Max push out eventualBG
                eBGweight = 0.75;
...
//            }
            ...
            if (ENWindowOK) {
                ...
                if (sens_predType.startsWith("UAM+")) ENMaxSMB = (firstMealWindow ? profile.EN_UAMPlus_maxBolus_bkfst : profile.EN_UAMPlus_maxBolus);
            } else {
