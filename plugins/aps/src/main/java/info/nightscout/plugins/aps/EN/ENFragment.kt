...
class ENFragment : DaggerFragment() {
    ...
    fun updateGUI() {
        ...
        val ENPlugin = activePlugin.activeAPS
        ENPlugin.lastAPSResult?.let { lastAPSResult ->
            binding.result.text = jsonFormatter.format(lastAPSResult.json)
            binding.request.text = lastAPSResult.toSpanned()
        }
        ENPlugin.lastDetermineBasalAdapter?.let { determineBasalAdapterENJS ->
            binding.glucosestatus.text = jsonFormatter.format(determineBasalAdapterENJS.glucoseStatusParam)
            binding.currenttemp.text = jsonFormatter.format(determineBasalAdapterENJS.currentTempParam)
            try {
                val iobArray = JSONArray(determineBasalAdapterENJS.iobDataParam)
        ...
        if (ENPlugin.lastAPSRun != 0L) {
            binding.lastrun.text = dateUtil.dateAndTimeString(ENPlugin.lastAPSRun)
        }
        ENPlugin.lastAutosensResult.let {
