...
//    var threshold = Math.max(min_bg-0.5*(min_bg-40), profile.normal_target_bg-9, 75); // minimum 75 or current profile target - 10
    ...
    try {
        iobArray.forEach(function(iobTick) {
            ...
            //IOBpredBG = IOBpredBGs[IOBpredBGs.length-1] + (round(( -iobTick.activity * sens_normalTarget / (Math.log(IOBpredBGs[IOBpredBGs.length-1]/ins_val)+1) * 5 ),2)) + predDev;
            // calculate predBGs with long zero temp without deviations
            var ZTpredBG = ZTpredBGs[ZTpredBGs.length-1] + predZTBGI;
            //var ZTpredBG = ZTpredBGs[ZTpredBGs.length-1] + (round(( -iobTick.iobWithZeroTemp.activity * sens_normalTarget / (Math.log(ZTpredBGs[ZTpredBGs.length-1] /ins_val)+1) * 5 ), 2));
            ...
            //UAMpredBG = UAMpredBGs[UAMpredBGs.length-1] + (round(( -iobTick.activity * sens_normalTarget / (Math.log(UAMpredBGs[UAMpredBGs.length-1] /ins_val)+1) * 5 ),2)) + Math.min(0, predDev) + predUCI;
    ...
    if (ENactive || ENSleepMode) {
        ...
        if (insulinReq_boost && insulinReq_bg <= target_bg) enableSMB = false;
